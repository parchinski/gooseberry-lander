...
const fragmentShader = `
  ...
  void main() {
    ...
    // Pure blue/accent dominant shader
    vec4 color = cppn_fn(uv, 0.1 * sin(0.2 * iTime), 0.1 * sin(0.5 * iTime), 0.1 * sin(0.3 * iTime));
    
    // Strong blue tint (#2563eb is roughly rgb(0.14, 0.39, 0.92))
    // We eliminate the purple/red contribution by zeroing out x/y components more aggressively before mixing
    gl_FragColor = mix(vec4(color.x * 0.2, color.y * 0.5, color.z, 1.0), vec4(0.14, 0.39, 0.92, 1.0), 0.5);
  }
`;
...
export function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="fixed inset-0 -z-50 w-full h-full bg-background" aria-hidden>
      <Canvas
        camera={camera}
